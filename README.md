...
> ⚠️ **No strategy has a validated edge (4 families tested, all failed).** The Phase A audit closed 2026-05-18 — `mean_reversion` dropped, `momentum` & `ORB` unvalidated. Two pre-registered Part 2 forks then ran and were killed: `intraday_continuation` (D3/BR3, **killed 2026-05-21**) and `news-proximity drift` (D2, **killed 2026-05-26**) — both failed OOS Deflated-Sharpe / CI / cost gates, both at the M15 intraday cost-tax. The *infrastructure* is complete and works; what is missing is a deployable strategy. **Do not trade this live.** Authoritative record + the next-phase strategy debate: [`docs/STRATEGY_AUDIT.md`](docs/STRATEGY_AUDIT.md).
...
**Strategy: none deployable.** The Phase A audit closed 2026-05-18 on the kill-criterion — no validated edge in `mean_reversion` (dropped), `momentum` or `ORB` (unvalidated). Two pre-registered Part 2 forks then ran and were killed: `intraday_continuation` (D3/BR3, Zarattini-inspired volatility-band continuation with a dynamic Chandelier trail, **killed 2026-05-21**) and `news-proximity drift` (D2, ForexFactory standardized-surprise + reaction-bar direction over a pre-registered {1,3,5 h} horizon grid, **killed 2026-05-26**) — both failed OOS DSR/CI/cost gates by structural margin, both at the M15 intraday cost-tax. Next-phase forks (B: longer horizon, C: lower-cost venue, D: Claude-as-co-pilot only) are open per [`docs/STRATEGY_AUDIT.md`](docs/STRATEGY_AUDIT.md) Part 2.
